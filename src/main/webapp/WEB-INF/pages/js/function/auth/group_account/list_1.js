...
function setTableClickEvent (tableName) {
	...
	$('table#dataTable tbody').on('click', 'td:not(:first-child)', function () {
		...
		ViewResolver.refresh('/Function/GroupAccount/editFlow', row);
	});
}
...
function setting () {
	return [ // 欄位設定
				...
				{targets: [1], title: '群組帳號', data: 'roleId', className: 'dt-nowrap'}, 
				{targets: [2], title: '群組名稱', data: 'roleName', className: 'dt-nowrap'}, 
				{targets: [3], title: '狀態 ', data: 'statusDisplay', className: 'dt-nowrap'}
			];
}
...
function del (action) {
	switch (action) {
		case '/Function/GroupAccount/delete':
			delet(action);
			break;
	}
}
