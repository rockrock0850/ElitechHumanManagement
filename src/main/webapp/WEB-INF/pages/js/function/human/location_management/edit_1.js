...
$(function() {
	...
	updateColumns('editLocationForm');
});

function updateColumns (name) {
	var location = JsonUtil.fromJson(locationJson);
	var locationId = location.locationId;
	var sales = location.sales;
	var locationName = location.locationName;
	var pm = location.pm;
	var workingTime = location.workingTime;
	var offTime = location.offTime;
	var lunchTime = location.lunchTime;
	
	$('label#locationId').html(locationId);
	...
	$('select[name=workingTime]').val(workingTime);
	$('select[name=offTime]').val(offTime);
	$('select[name=lunchTime]').val(lunchTime);
}

function edit (action) {
	var result = Validator.input('editLocationForm', [], {
		locationName: '駐點名稱',
		sales: '業務',
		pm: '駐點PM'
	}, true);
	...
	var data = form2object('editLocationForm');
	JsonUtil.post(action, data, function (resData) {
		bootbox.alert({
			size: 'small',
			message: resData.msg, 
			callback: function () {
				ViewResolver.refresh('/Function/LocationManagement/queryList', resData.data);
			}
		});
	});
}
...
function update (action) {
	switch (action) {
		case '/Function/LocationManagement/editFlow/edit':
			edit(action);
			break;
	}
}
